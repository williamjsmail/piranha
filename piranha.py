...
from PyQt6.QtWidgets import (
    ...
    QSlider, QScrollArea, QDialogButtonBox, QToolBox
)
from PyQt6.QtGui import QAction, QWheelEvent, QMouseEvent, QPainter, QIntValidator, QIcon
...
from backend.cve_tech_mapper import CveTechniqueMapperDialog
...
class PiranhaApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Piranha v3.4.1")
        self.adjustSize()
        ...
        cve_map_tool = QAction("Quick CVE Mapper", self)
        cve_map_tool.triggered.connect(self.open_cve_mapper)
        tool_menu.addAction(cve_map_tool)
        update_cve_db = QAction("Update CVE Database", self)
        update_cve_db.triggered.connect(self.update_cve_db)
        tool_menu.addAction(update_cve_db)
    ...
    def init_analysis_tab(self):
        ...
        self.search_bar.setFixedWidth(520)
        self.search_bar.textChanged.connect(self.filter_table)
        dataset_layout.addWidget(self.search_bar)
        self.filter_btn = QPushButton()
        self.filter_btn.setIcon(QIcon.fromTheme("edit-find"))  # This uses system theme icon
        self.filter_btn.setFixedSize(28, 28)
        self.filter_btn.setToolTip("Filter Report Table")
        dataset_layout.addWidget(self.filter_btn)
        self.filter_btn.clicked.connect(self.launch_unified_filter)
        ...
        generate_btn.clicked.connect(lambda: self.handle_generate_report(
            self.apt_listbox,
            self.tactic_listbox,
            self.TACTIC_MAPPING,
    ...
    def handle_generate_report(self,apt_listbox,tactic_listbox,TACTIC_MAPPING,include_descriptions,use_enterprise,use_mobile,use_ics,include_detections,table,graph_view,radar_fig,radar_canvas,radar_summary,radar_tables):
        self.original_table_data = generate_report(
            apt_listbox,
            tactic_listbox,
            TACTIC_MAPPING,
    ...
    def view_full_row(self):
        ...
        column_headers = ["APT", "Category", "T-Code", "Dataset Source", "Description", "IOM", "Detection Tool", "MITRE Detection"]
    ...
    def launch_unified_filter(self):
        if not hasattr(self, "original_table_data"):
            return

        column_names = ["APT", "Category", "T-Code", "Dataset Source", "Description", "IOM", "Detection Tool", "MITRE Detection"]
        column_data_map = {}
        for i, col in enumerate(column_names):
            column_data_map[col] = []
            for row in self.original_table_data:
                if i < len(row):
                    column_data_map[col].append(row[i])
                else:
                    column_data_map[col].append("")

        filter_dialog = UnifiedFilterDropdown(column_data_map, self.apply_unified_filter)
        filter_dialog.exec()
    
    def apply_unified_filter(self, selected_filters):
        self.table.setRowCount(0)

        for row_data in self.original_table_data:
            match = True
            for category, selected_values in selected_filters.items():
                column_names = [self.table.horizontalHeaderItem(i).text() for i in range(self.table.columnCount())]
                col_index = column_names.index(category)
                if str(row_data[col_index]) not in selected_values:
                    match = False
                    break
            if match:
    ...
    def open_cve_mapper(self):
        dialog = CveTechniqueMapperDialog(self)
        dialog.exec()
    ...
    def update_cve_db(self):
        try:
            ...
            scripts = [
                os.path.join(base_path, "CVE2CAPEC", "retrieve_cve.py"),
                os.path.join(base_path, "CVE2CAPEC", "cve2cwe.py"),
                os.path.join(base_path, "CVE2CAPEC", "cwe2capec.py"),
                os.path.join(base_path, "CVE2CAPEC", "capec2technique.py")
            ]
            messages = []
            ...
            for script_path in scripts:
                ...
                result = subprocess.run(
                    ...
                    cwd=script_dir,  # run from within the script's folder
                    capture_output=True,
                    text=True
                )

                if result.returncode != 0:
                    messages.append(f"❌ {os.path.basename(script_path)} failed:\n{result.stderr.strip()}")
                else:
                    messages.append(f"✅ {os.path.basename(script_path)} completed successfully.")

            summary = "\n\n".join(messages)
            QMessageBox.information(self, "CVE Database Update Status", summary)

        except Exception as e:
            QMessageBox.critical(self, "Update Error", f"An unexpected error occurred:\n{str(e)}")
...
"<b>Manage Keywords:</b> The <b>Manage Keywords</b> button will allow you to add or edit existing mappings for keywords → IOMs → detection tools. This feature allows your team to dynamically update your data to stay current with how you hunt, as well as what you hunt on.<br><br>"
...
class UnifiedFilterDropdown(QDialog):
    def __init__(self, column_data_map, apply_callback):
        super().__init__()
        self.setWindowTitle("Filter Report Table")
        self.resize(900, 500)
        ...
        for category, values in column_data_map.items():
            ...
            self.filter_lists[category] = list_widget
            self.tool_box.addItem(list_widget, category)

        scroll_layout.addWidget(self.tool_box)
        scroll_area.setWidget(scroll_content)
        main_layout.addWidget(scroll_area)

        # OK / Cancel buttons
        buttons = QDialogButtonBox(QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
        buttons.accepted.connect(self.apply_filters)
        buttons.rejected.connect(self.reject)
        main_layout.addWidget(buttons)

    def apply_filters(self):
        self.selected_items = {}
        for category, list_widget in self.filter_lists.items():
            selected = []
            ...
            if selected:
                self.selected_items[category] = selected
        self.apply_callback(self.selected_items)
        self.accept()
